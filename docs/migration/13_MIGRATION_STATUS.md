...
## FAZA 3w-4 - KRITICNO: neposlate izmene + Knjiga detalj-modal + preostale sitne ispravke (2026-07-12, Claude/Cowork sesija)

Status: DONE (implementacija + commit), CEKA korisnicko `git push` (sandbox nema mrezni pristup ka GitHub-u)

Kontekst: korisnik je posle FAZA 3w-2/3w-3 prijavio da SVE navedene ispravke (fiksna dugmad na Nalogu za isplatu/Zahtevima/Smeni, oko-dugme na Zahtevima) izgledaju kao da uopste nisu primenjene - "kao da nije bilo izmene", verzija i dalje 0.5.0.

### KRITICAN nalaz - koren SVIH "ništa se nije promenilo" prijava

`git status` je pokazao da su SVE izmene iz FAZA 3w-2 i FAZA 3w-3 (i ranije) stajale NEKOMITOVANE u lokalnom radnom drvetu - poslednji komit je bio "CLAUDE14", stariji od bilo koje izmene ove sesije. Ako Cloudflare Pages deploy-uje sa git push-a, korisnik je CEO OVO VREME testirao stari build sa GitHub-a, ne lokalne izmene - ovo objasnjava BAS SVE prijavljene "kao da nije bilo izmene" simptome odjednom (Nalog za isplatu dugmad, Zahtevi oko, Smena, verzija 0.5.0).

Dodatno je pronadjen zaglavljen `.git/index.lock` (prazan, iz prekinute ranije git operacije) koji je blokirao normalno `git add`/`git commit` - zaobidjeno preko privremenog `GIT_INDEX_FILE` (izvan `.git` direktorijuma) da se izbegne zakljucana putanja.

**Uradjeno u ovoj sesiji**: verzija bump-ovana na `cloudflare-migration-0.6.0`, sve izmene dodate i komitovane (`git commit`, hash `c038de0`, poruka "CLAUDE15: FAZA 3w-4 ..."). **`git push origin main` NIJE uspeo** - sandbox mrezni proxy vraca `403` na konekciju ka `github.com` (mrezna politika ovog Cowork sandboxa, ne git/auth problem). **Komit postoji lokalno u repozitorijumu na korisnickom racunaru** (`C:\Users\milan\source\repos\BLAGAJNA_WEB`), ali korisnik mora sam da pokrene `git push origin main` (ili ekvivalentno kroz svoj git klijent/IDE) da bi izmene stigle na GitHub i pokrenule Cloudflare Pages deploy. Bez ovog koraka, sve dalje navedene ispravke (i sve iz FAZA 3w-2/3w-3) ostace nevidljive na produkciji.

### Ostale ispravke ove runde

1. **Knjiga - detalji stavke premesteni u modal** (`scripts.html` `openDetailPanel_`, `renderKnjigaTable_`, `renderKnjigaRow_`; `desktop.html`) - korisnik: "detalje ukloniti iz sada postojece pozicije... otvoriti jasnu preglednu formu sa svim detaljima". `openDetailPanel_` vise ne popunjava uski staticni bocni panel (`#d-detail-panel`) nego gradi `.app-dialog-overlay` modal (isti obrazac kao `openAnnouncementDetail_`), sa svim starim poljima (hero, detalj-grid, istorija stanja) i svim dugmadima (Priznanica/Dokumenti/Blagajnicki list/Presek blagajne/Storno, sad `cash-count-action` klase). `#d-detail-panel` je trajno sakriven (`hidden` atribut u `desktop.html`) - "Brze akcije" (Uplata/Isplata/Trezor/Najava) ostaju JEDINI, i time garantovano fiksni, sadrzaj bocnog panela. Dodata kolona "Akcije" sa "oko" dugmetom (`renderKnjigaRow_`) u desktop tabelu - klik NA RED vise ne otvara detalje (samo "oko"). Mobilni prikaz NIJE menjan (tap-na-karticu i dalje otvara `openMobileDetailPanel_`, koji je vec puni preko-cele-strane overlay - to nije bila korisnikova primedba).
2. **Zahtevi - "oko" dugme bez vidljive potvrde** (`scripts.html` `renderPaymentRequestTable_`) - klik je bio ispravno ozicen (`renderPaymentRequestDetail_`), ali kad je red vec bio izabran panel se re-renderovao identicnim sadrzajem pa je klik izgledao kao da "ništa se ne dešava". Dodato: klik sad UVEK skroluje detalj-panel u prikaz i kratko ga vizuelno istakne (`.flash-highlight`, nova CSS animacija u `styles.html`) - garantovana potvrda bez obzira na to da li se sadrzaj promenio.
3. **Presek stanja - "Novi presek" boja** (`desktop.html`) - dugme je koristilo samo `.presek-primary-action` (plava, `var(--color-primary)`) umesto zelene konvencije za "unos novog dokumenta" koriscene svuda drugde (npr. "Novi nalog za isplatu" = `cash-count-action save`). Dodata klasa `cash-count-action save` (zelena, ista kao ostale "Novi ..." akcije).
4. **Nalog za isplatu/Zahtevi/Smena fiksna dugmad** - kod je PONOVO pregledan (konsolidovano pravilo iz FAZA 3w-3, `.po-panel-actions`/`.shift-actions-grid` itd.) - nije pronadjen nikakav preostali CSS konflikt; najverovatniji uzrok "i dalje se pomeraju" je isti KRITICAN nalaz iznad (neposlat kod), ne nova greska.
5. **Mobilna Najava uplate za ANNOUNCER** - `restrictNavToNajave_`/`bindAnnouncementButtons_`/privilegijske funkcije ponovo pregledane - kod ostaje ispravan (`m-fab-najava` nije u listi za sakrivanje, `canViewPaymentAnnouncements_`/`canCreatePaymentAnnouncement_` vracaju tacno za ANNOUNCER); najverovatniji uzrok je isti KRITICAN nalaz iznad.

### Sta NIJE uradjeno / ostaje otvoreno

- **`git push` mora korisnik rucno da odradi** - vidi KRITICAN nalaz iznad. Dok se ovo ne uradi, NIJEDNA izmena (ni ova ni prethodne dve faze) nece biti vidljiva na produkciji.
- Runtime test na uredjaju/produkciji nije radjen - verifikacija je citanje izmenjenog koda.
- Uzrok ZASTO je toliko izmena ostalo neposlato kroz vise sesija zaredom nije istrazen (van dometa - vezano za radni tok van ove alatke).

Sledeci korak (korisnik): **1) `git push origin main`** (ili ekvivalentno) da bi Cloudflare Pages preuzeo sve izmene od FAZA 3w-2 naovamo; 2) tek POSLE deploy-a ponovo proveriti sve prijavljene stavke (verzija treba da pokaze `0.6.0`, Knjiga oko-modal, Zahtevi oko-flash, Novi presek zelen, fiksna dugmad svuda, mobilna Najava za ANNOUNCER).

## FAZA 3w-5 - Stvarni uzrok "dugmad skacu" (Nalog za isplatu) + mobilna Najava odbrana (2026-07-12, Claude/Cowork sesija)

Status: DONE (implementacija), CEKA korisnicku potvrdu na uredjaju + git push

Kontekst: korisnik je posle deploy-a verzije 0.6.0 potvrdio da je build svez (verzija se vidi), ali da dugmad JOS UVEK skacu/nisu zakljucana, i da mobilna Najava za ANNOUNCER i dalje radi nekonzistentno (ponekad se ne prikaze na prvo ucitavanje, tek posle par klikova). Poslat je i novi video snimak.

### Uzrok #1 - "2 vs 3 dugmeta po redu" (flexbox specificnost, ne redosled)

Analizom snimka (crop tacne granice dugmadi, ne samo procena) potvrdjeno da su Nalog za isplatu/Zahtevi/Smena i dalje prikazivali 3 (ili promenljivo) dugmeta po redu umesto ocekivana 2, uprkos FAZA 3w-3 pravilu `.po-panel-actions button { flex: 1 1 calc(50% - 5px); }`. Koren: FAZA 3w-2 konsolidovano pravilo `.po-panel-actions .cash-count-action { ...; width: auto; }` ima VISU CSS specificnost (dve klase, 0-2-0) od `.po-panel-actions button` (klasa+element, 0-1-1) - to je DRUGO svojstvo (`width` naspram `flex`/`flex-basis`) i teorijski `flex-basis` treba da ima prednost u flex algoritmu, ali kombinacija dva razlicita mehanizma (flex-basis iz jednog pravila, width iz drugog, razlicite specificnosti) pokazala se nepouzdanom/nepredvidivom u praksi - sto je i bio ceo problem sa "dugmad se ponasaju drugacije nego sto CSS pravila same po sebi izgledaju da bi trebalo".

**Ispravka**: `.po-panel-actions` (i `.shift-actions-grid`, koji nasledjuje isti sistem) presvuceni sa flexbox na **CSS Grid** (`display:grid; grid-template-columns: repeat(2, minmax(0,1fr))`). Grid ne ostavlja prostor za ovu dvosmislenost - broj kolona je eksplicitan i nezavisan od bilo kog drugog `width`/`flex` pravila, i grid podrazumevano razvlaci (`align-items:stretch`) SVE celije u redu na visinu najvise, pa je i "razlicita visina dugmadi" resena bez potrebe za `max-height`/`overflow:hidden` (koji je ranije mogao da odseca duzi tekst). `.po-panel-actions button { flex: ... }` pravilo i `flex-wrap:wrap` su uklonjeni (nepotrebni za grid). Header-kontekst dugmad (`.po-head-actions`/`.new-order-titlebar`/`.cashbook-lock-note`/`.shifts-page-head` - mesta sa samo 1-2 dugmeta pored naslova, ne "traka") OSTAJU na flex+width:auto+nowrap kao ranije, jer se tamo dvosmislenost ne javlja (nema `.po-panel-actions button` konkurentskog pravila).

### Uzrok #2 - Nalog za isplatu NIKAD nije dobio "fiksni panel" tretman (pravi uzrok jumping-a)

Utvrdjeno da `.po-detail-panel` (Nalog za isplatu, `desktop.html` `#d-order-detail-panel`) NIKADA nije dobio flex-kolonu + ogranicenu visinu + "samo unutrasnji sadrzaj skroluje" tretman koji su Zahtevi (`.payment-requests-page .po-detail-panel`) i Korisnici (`.users-admin-page .po-detail-panel`) dobili jos u FAZA 3w-2 - postojao je samo generican `.po-detail-panel { min-height:580px }` BEZ ikakvog ogranicenja visine/skrola. Posledica: ceo `.d-content` (spoljna skrol oblast cele stranice) se skrolovao zajedno sa "Tok naloga" istorijom, cija duzina varira od naloga do naloga - zbog toga se dugme "Posalji na isplatu" nalazilo na RAZLICITOJ apsolutnoj poziciji na ekranu za svaki nalog, sto je izgledalo kao "dugmad skacu" pri prebacivanju izmedju naloga (a ne stvarno "skakanje" unutar iste stavke).

**Ispravka**: `renderPaymentOrderDetail_` (`scripts.html`) sada omotava sredisnji sadrzaj (polja/iznos/tabovi/tok naloga) u nov `<div class="po-order-detail-scroll">`, van kog ostaju zaglavlje (`.po-detail-head`) i dugmad (`.po-panel-actions`). Novo CSS pravilo `.payment-orders-page .po-detail-panel { display:flex; flex-direction:column; max-height:calc(100dvh - 260px); overflow:hidden }` + `.payment-orders-page .po-order-detail-scroll { flex:1; min-height:0; overflow:auto }` - identican obrazac kao Zahtevi/Korisnici. Dugmad sad ostaju na KONSTANTNOJ poziciji bez obzira na duzinu istorije naloga.

### Uzrok #3 - `.shift-actions-grid` sticky sa negativnim `bottom`

`.shift-actions-grid` je koristio `position:sticky; bottom:-14px` - negativna vrednost gura element 14px ISPOD donje ivice scroll kontejnera, sto moze da spreci ispravno lepljenje ili delimicno odsece traku pri skrolovanju. Ispravljeno na `bottom:0` (standardan nacin lepljenja uz donju ivicu).

### Mobilna Najava za ANNOUNCER - odbrambeno pojacanje (nije pronadjen logicki bag)

Detaljno ponovo pregledana cela putanja (`restrictNavToNajave_`, `bindAnnouncementButtons_`, `canViewPaymentAnnouncements_`, `canCreatePaymentAnnouncement_`, redosled `applyAppSession_` → `bootstrapShellAfterAppSession_`, backend `verifySession`/`sanitizeUser` - privilegije se ispravno vracaju u `app_user.privileges`) - kod je logicki ispravan, nije pronadjen konkretan bag koji bi objasnio povremeno "ne prikaze se na prvo ucitavanje". Kako nije bilo moguce reprodukovati u browser konzoli (van dometa ove sesije/sandboxa - nema live browser pristupa), primenjena je odbrambena ojacanja umesto tacne popravke:
- Redirekt/restrikcija za ANNOUNCER/ASSISTANT_CASHIER (`applyAnnouncerRestriction_` u `bootstrapShellAfterAppSession_`) se sad PONAVLJA jednom posle 600ms (setTimeout) - ako je prvi prolaz vec uspeo, ovo je bezopasno ponavljanje.
- `startLiveRefresh_` (15s interval) sada NA SVAKOM tick-u ponovo primenjuje `restrictNavToNajave_()` i proverava da li je `m-section-najave` aktivna sekcija za ovu ulogu (ako nije, ponovo je otvara) - garantovana treca/cetvrta... prilika da se ispravi bez potrebe da korisnik rucno klika okolo. Ovo takodje uklanja nepotrebne (i za ovu ulogu 403-ujuce) pozive `refreshBalance_`/`loadKnjiga_`/`loadMobileNalozi_` iz tog tick-a.

Ako se problem i dalje javlja POSLE ove izmene, potrebna je stvarna browser konzola (F12 → Console/Network) u trenutku kad se greska desi da bi se video tacan uzrok - staticka analiza koda je iscrpljena bez novih tragova.

### Sta NIJE uradjeno / ostaje otvoreno

- Tacan uzrok mobilne Najava nekonzistentnosti NIJE potvrdjen (samo odbrana/ublazavanje) - potrebna je browser konzola za precizniju dijagnozu ako se ponovi.
- Runtime test na uredjaju/produkciji nije radjen - verifikacija je citanje izmenjenog koda + analiza snimka ekrana (crop poredjenje piksela).
- `git push` i dalje mora korisnik rucno da odradi (sandbox nema mrezni pristup ka GitHub-u) - napomena: korisnik je u prethodnoj rundi VEC sam komitovao i verovatno pushovao (log pokazuje njihov commit "VLAUDE16" posle moje poruke), pa ce i ova runda verovatno zahtevati isti rucni korak.

Sledeci korak (korisnik): 1) `git push` (ako vec nije uradjeno za ovu rundu), 2) vizuelno potvrditi da su dugmad na Nalogu za isplatu/Zahtevima/Smeni sad TACNO 2 po redu i da ostaju na ISTOJ poziciji bez obzira koji nalog/zahtev/smena se gleda, 3) ponovo testirati mobilnu Najavu za ANNOUNCER na svezem ucitavanju (idealno vise puta zaredom) - ako se JOS UVEK povremeno ne prikaze, potrebno je poslati screenshot browser konzole (F12) u tom trenutku.
 pronadjen (samo je greska sad vidljiva korisniku umesto da se cutke gubi).
- Root uzrok "korisnik upisan kao blagajna" u Blagajnickom listu nije pronadjen u kodu (posted_by/created_by su svuda ispravno vezani za appUser.email) - potrebna konkretna stavka za dalju istragu ako se ponovi.
- Desktop nav restrikcija (`restrictNavToNajave_`) sakriva dugmad, ali ne testira svaki pojedinacni backend endpoint da li ADEKVATNO 403-uje ANNOUNCER/ASSISTANT_CASHIER - oslanja se na vec postojecu privilegijsku proveru po endpoint-u (nije revidirano u ovoj sesiji).
...
## FAZA 3w-3 - Konzistentnost dugmadi i mobilni FAB privilegije (2026-07-12, Claude/Cowork sesija, posle video snimka)

Status: DONE (implementacija), CEKA vizuelnu potvrdu korisnika

Kontekst: korisnik je poslao snimak ekrana posle FAZA 3w-2 - dugmad se i dalje razlikuju po stranicama (velicina, boldovanost, oblik), a na mobilnom ANNOUNCER i dalje moze da otvori "Novi nalog za isplatu" i "Presek stanja" iz "+" menija iako mu nav tabovi za te sekcije NISU vidljivi.

### Koren problema (pregledano frejm-po-frejm iz snimka)

1. **`.shift-actions-grid` (Smena) je imao SVOJ `display:grid` layout, ali je HTML dugmadima naknadno dodata i klasa `.po-panel-actions` (radi boje/fonta) - a `.po-panel-actions{display:flex}` je definisan KASNIJE u `styles.html` pa je (ista specificnost, redosled odlucuje) preglasio `display:grid`.** Rezultat: Smena se stvarno rasporedjivala po flex-wrap pravilima (koja su bila podesena za Zahtevi-jevih 6 dugmadi), ne po nameravanom 3-kolonskom gridu - otud "4 u jednom redu, 1 samo u drugom" umesto ocekivanog rasporeda, drugacijeg od svake druge stranice.
2. **Duzi natpisi ("Odobri i kreiraj nalog", "Odbij i kreiraj nalog") su se prelamali u 2 reda dok kraci natpisi ("Odbij", "Vrati na dopunu") ostaju u 1 redu** - u istoj traci dugmadi to daje vizuelno neujednacenu visinu/izgled po stranici do stranice (Nalog za isplatu retko prelama jer ima kraca dugmad, Zahtevi skoro uvek prelama).
3. **Najava - detalj dijalog (`openAnnouncementDetail_`) je mesao DVA razlicita sistema dugmadi u ISTOJ traci**: "Izmeni"/"Vrati na doradu"/"Detalji uparivanja"/"Dokumenti" su imali `btn-ghost` klasu (nasledjuje generican `.m-detail-actions button` stil - 64px visok, 19px font, bold 900), dok "Pošalji u blagajnu"/"UPLATA"/"Otkaži najavu" imaju `cash-count-action` klasu (44px, 13px) - DVA POTPUNO RAZLICITA dugmeta rame uz rame u istom redu. Ovo je bio konkretan, potvrdjen uzrok "Dokumenti izgleda drugacije od Detalji uparivanja".
4. **Mobilni FAB "+" meni nije bio pokriven `restrictNavToNajave_()`** - ta funkcija je sakrivala samo `[data-d-nav]`/`[data-m-nav]` (nav tabove), ali FAB meni (`m-fab-new-order`, `m-fab-new-count`, `m-btn-uplata`, `m-btn-isplata`, `m-btn-trezor`, `m-fab-mandatory-count`) je poseban, uvek-prisutan plutajuci meni nezavisan od toga koji je nav tab aktivan - ANNOUNCER/ASSISTANT_CASHIER ga je i dalje video i mogao njime da otvori zakljucane sekcije.

### Ispravke

- **`styles.html`**: `.shift-actions-grid` vise NE definise `display`/`grid-template-columns`/`gap` (uklonjeno, ukljucujuci mrtvo pravilo u `@media (max-width:1180px)`) - u potpunosti se oslanja na `.po-panel-actions` (ista klasa kao Zahtevi/Nalog za isplatu/Korisnici), ostaju samo "sticky footer" specificna svojstva (pozicija/pozadina). Ovo je JEDAN sistem umesto dva koja se sudaraju po redosledu u fajlu.
- **`styles.html`**: konsolidovano pravilo za SVE akcione trake (`.po-panel-actions/.po-head-actions/.new-order-titlebar/.cashbook-lock-note/.shifts-page-head/.shift-actions-grid .cash-count-action`) - `white-space:nowrap` (tekst se VISE NIKAD ne prelama), `min-height`/`max-height` fiksno na 44px (identicno svuda, bez obzira na duzinu teksta), font smanjen na 12.5px i gap na 6px da duzi natpisi staju u 1 red. Dodatno, `.po-panel-actions button` sad ide 2 dugmeta po redu (`flex: 1 1 calc(50% - 5px)`, bilo 3 po redu na 140px bazi) - sire dugme, manje prelamanja, i USKLADJENO ponasanje na SVIM stranicama (Nalog za isplatu 4 dugmeta sad ide 2x2 red umesto 3+1, isto kao Zahtevi/Smena/Korisnici).
- **`scripts.html` (`openAnnouncementDetail_`)**: SVA dugmad u Najava detalj dijalogu (Izmeni/Pošalji u blagajnu/UPLATA/Vrati na doradu/Detalji uparivanja/Dokumenti/Otkaži najavu) sad koriste ISKLJUCIVO `cash-count-action draft/save/cancel` (boja po semantici: draft=informativno/sekundarno, save=pozitivna/primarna akcija, cancel=destruktivno), i dodata je `.po-panel-actions` klasa na kontejner da nasledi konsolidovano pravilo iznad.
- **`scripts.html` (`restrictNavToNajave_`)**: dodato eksplicitno sakrivanje `m-fab-new-order`/`m-fab-new-count`/`m-fab-mandatory-count`/`m-btn-uplata`/`m-btn-isplata`/`m-btn-trezor` za ANNOUNCER/ASSISTANT_CASHIER (FAB meni je odvojen od nav tabova, nije bio pokriven ranijom verzijom ove funkcije). `m-fab-najava` OSTAJE vidljiv - pokriven je posebnom `canCreatePaymentAnnouncement_()` proverom.

### Sta NIJE menjano (namerno)

- Modalne forme za kreiranje/izmenu (Cancel=`btn-ghost` + Submit=`btn-primary`, npr. forma za novu najavu, forma za UPLATA/knjiženje) NISU prebacene na `cash-count-action` - taj par se koristi identicno u DESETINAMA drugih modala u celoj aplikaciji (korisnici, valute, presek...) i vec je medjusobno konzistentan (isti bazni `button` stil - 34px, 13px, weight 700, samo boja se menja). Menjanje SAMO za najavu bi napravilo NOVU nekonzistentnost umesto da je resi.
- Nije bilo moguce vizuelno potvrditi svaki ekran 1:1 (analiza je radjena citanjem izvucenih frejmova iz poslatog snimka + citanjem CSS/JS izvora, ne live testom na uredjaju).

Sledeci korak (korisnik): vizuelno potvrditi da su Nalog za isplatu/Zahtevi/Smena/Korisnici/Najava detalj dugmad sada IDENTICNog oblika/boje/visine (2 po redu, bez prelamanja teksta), i da ANNOUNCER na mobilnom vise NEMA pristup "Novi nalog"/"Presek stanja"/"Uplata"/"Isplata"/"Trezor" iz "+" menija.

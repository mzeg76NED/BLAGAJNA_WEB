...
## FAZA 3w - Desktop regresija, mobilna dorada FAB/header-a, Knjiga/Blagajnicki list numeracija, i Najava uplate radni tok (2026-07-11, Claude/Cowork sesija)

Status: DONE (implementacija), CEKA korisnicku potvrdu + primenu SQL migracije na Supabase

Kontekst: posle FAZA 3v korisnik je prijavio (1) desktop regresiju - posle logovanja se ne osvezavaju podaci/aktivna smena/oznaka verzije; (2) niz mobilnih dorada ("+" mora da lebdi, hamburger premalen, ukloniti Promeni korisnika i rucne refresh dugmiće, obavezan presek premestiti u "+"); (3) bagove u Blagajnickoj knjizi i Blagajnickom listu (numeracija racuna od juce umesto od 1 po danu, uparene najave iz prethodnog dana se probijaju kroz filter, KPI najave ne postuje aktivan period, pogresan korisnik u koloni); (4) veliki zahtev za rework celog "Najava uplate" radnog toka za ANNOUNCER ulogu (nacrt → slanje u blagajnu → uparivanje/vracanje na doradu), sa dokumentima, filterima, KPI i stampom.

### 1. Desktop regresija (bootstrap)

- **`src/html/scripts.html`** - dodat `safeStep_(label, fn)` wrapper (try/catch + `console.error`) i `bootstrapShellAfterAppSession_` prepravljen da SVAKI korak zove kroz `safeStep_(...)` umesto direktno. Ranije je jedan neuhvacen izuzetak u ranijem koraku (npr. unutar `initializeSession`) prekidao CEO ostatak sinhronog niza poziva - ukljucujuci `renderAppVersion_()`, `applyActiveShiftFields_()` i `loadKnjiga_(true)` koji su svi kasnije u istom lancu. Ovo objasnjava sva tri prijavljena simptoma odjednom (nema oznake verzije, ne ucitava aktivnu smenu, Knjiga se ne osvezava bez rucne izmene datuma). Tacan uzrocni red nije mogao da se potvrdi bez browser konzole (van dometa ove sesije), pa je primenjena opsta otpornost koja pokriva CELU klasu ovakvih bagova, ne samo trenutni uzrok.
- **`web/public/cloudflare-apps-script-adapter.js`** - `appVersion` bump-ovan na `cloudflare-migration-0.5.0` (bio hardkodovan i nikad azuriran - sad vizuelno potvrdjuje da je deploy svez).

### 2. Mobilna dorada (FAB, hamburger, header, obavezan presek, uklonjena dugmad)

- **`.m-fab-row`** (`styles.html`) - promenjeno iz `position:relative` flex sibling-a (FAZA 3v) u `position:fixed` (dole desno, `bottom:max(14px, safe-area-inset-bottom)`) - "+" sada STVARNO lebdi iznad sadrzaja umesto da zauzima red u flex layout-u i "jede" deo ekrana. Ovo NIJE isti anti-pattern kao raniji `position:fixed` bagovi (FAZA 3q/3t) jer ovde nema trake ispod koja bi se sudarila - to je objasnjeno komentarom u kodu da se ne pomesa sa tom ranijom klasom bagova. `.m-content` padding-bottom povecan 16px→84px da sadrzaj ne bude prekriven plutajucim dugmetom.
- **`.m-hamburger-btn`** (`styles.html`) - uvecano (font-size 17→23px, 26px→34px kutija) - "Hamburger je premali, sama oznaka mora biti malo veca".
- **Header dodatnih -30%** (`styles.html`, sve tri paralelne definicije: bazna, `html.mobile-view`, `@media`) - `.m-header` padding 9/7px → 6/5px, `.m-header-top` gap 8→6px, `.m-header-kpi` margin-top 7→5px. SAMO razmaci/padding su smanjeni (visina trake), font/ikonice unutra NISU dirani ("elemente ostavi iste velicine").
- **"Obavezan presek"** (`mobile.html`/`scripts.html`) - stara plutajuca pilula (admin-hint) je na mobilnom PREMESTENA u "+" meni kao novo dugme `#m-fab-mandatory-count` (prikazuje se/skriva JS-om preko `bindMobileMandatoryCountFabItem_`, samo ADMIN/DIREKTOR kad blagajna NIJE zakljucana). `renderMandatoryCountBar_()` sad detektuje mobilni shell (`document.querySelector('.m-shell')`) u admin-hint grani i PRESKACE renderovanje stare trake tamo (`bar.hidden = true`), dok desktop ponasanje ostaje potpuno nepromenjeno.
- **Uklonjeno sa mobilnog** (`mobile.html`): `#m-knjiga-refresh` i `#m-count-refresh` dugmad ("osveži ukloni, podatke osveži normalno sam" - JS listeneri su vec bili null-safe, sigurno uklanjanje) i `#app-switch-user-btn` iz hamburger fioke ("postoji odjava, pa ce na prijavi da se promeni korisnik - logicno"; desktop i dalje ima svoj `#app-switch-user-btn`, potpuno odvojen element).

### 3. Blagajnicka knjiga i Blagajnicki list - numeracija i najave

- **Numeracija od 1 po danu** (`renderKnjigaTable_` u `scripts.html`, i `assignDailyEntryNumbers()` u novom `_lib/cashSheet.js`) - Rb. kolona je RANIJE prikazivala stabilan globalan `ref_no` (Knjiga) odnosno globalan indeks kroz CELU istoriju (Blagajnicki list), pa je brojanje "nastavljalo od juce" cak i kad je prikaz ogranicen na danasnji dan. Sad se racuna NOV, cisto prikazni `entry_number`/`__displayIndex` koji se resetuje na 1 za svaki kalendarski dan u trenutno ucitanom/prikazanom periodu (najave imaju svoju odvojenu numeraciju, ne ulaze u dnevni brojac). Stabilan `ref_no` i dalje postoji nepromenjen i koristi se za sve cross-reference potrebe (Storno stavke #N, po najavi #N, naslov dijaloga dokumenata) - ovo je CISTO prikazna izmena, ne dira identifikatore.
- **Najave koje probijaju filter** (`scripts.html`) - `mergeAnnouncementsIntoEvents_`/`computeAnnouncementTotals_` sad postuju aktivan datumski opseg Knjige (`LAST_KNJIGA_DATE_RANGE`, postavlja ga `loadKnjiga_`) preko nove `announcementInActiveRange_()` - ranije je `apiListPaymentAnnouncements` ucitavao SVE najave bez datumskog filtera pa su se UPARENE najave iz prethodnih dana prikazivale u Knjizi bez obzira na filter, a KPI polje NAJAVE je sumiralo sve ucitano umesto samo aktivnog perioda.
- **Otvori smenu dugme** (`desktop.html`) - premesteno iz bocnog panela (`.cashbook-lock-note`, prikazivalo se samo kad nema smene) u gornji filter red pored dugmeta za osvezavanje ("gore desno"); vidljivost/disabled logika (`updateDesktopDirectActionsVisibility_`) ostaje ista, samo je markup lokacija promenjena.
- **Naslov velikim slovima** - "Blagajnicka knjiga" → "BLAGAJNICKA KNJIGA" u `desktop.html`.
- **Korisnik u Blagajnickom listu** - `posted_by`/`created_by` u `_lib/cashCounts.js`/`inflow.js`/itd. su vec ispravno postavljani na stvarnog logovanog korisnika (provereno citanjem koda) - nije pronadjen kod koji bi upisivao generican "blagajna" string. Ako se problem i dalje ponavlja posle ove izmene, verovatno je vezan za konkretan nalog kojim se korisnik prijavljuje (npr. deljeni login), ne za grešku u kodu - potrebna je konkretna stavka za dalju istragu.
- **Neupisan "pocetak dana" posle otvaranja smene** - istrazeno: `apiOpenShiftWithOpeningCount` upisuje pocetni presek "best-effort" (ako upis preseka ne uspe, smena ostaje otvorena ali se GRESKA CUTKE gutala, korisnik nije imao pojma da presek nije sacuvan). Dodato: `warnIfOpeningCountSkipped_()` na sva 3 mesta gde se ovaj API poziva - ako `result.countSkipped`, korisniku se sad prikazuje eksplicitna poruka gresku da presek NIJE sacuvan i da ga treba rucno uraditi u "Presek stanja". Sam uzrok ZASTO upis preseka ponekad ne uspe nije pronadjen (zahteva browser konzolu za tacnu gresku), ali sada se bar vidi kad se to desi.

### 4. Najava uplate - rework radnog toka (DRAFT → OPEN → MATCHED/RETURNED/CANCELLED)

Najveca izmena ove sesije. Ranije: najava je odmah pri kreiranju bila status `OPEN` (vidljiva blagajniku/Knjizi). Sada: kreiranje uvek ide u `DRAFT` (vidljivo SAMO autoru, ne proizvodi nikakvu akciju), autor je moze menjati/pregledati, i tek eksplicitnim "Posalji u blagajnu" prelazi u `OPEN` (vidljivo/uparljivo). Blagajnik (payment_announcements:match) moze umesto uparivanja da je vrati autoru na doradu (`RETURNED` - ponovo editabilna, mora se ponovo poslati). Otkazivanje je moguce u DRAFT/OPEN/RETURNED, ali ne posle MATCHED.

- **SQL** (`supabase/migrations/202607090001_initial_schema.sql`, dodat na kraj fajla, isti idempotentni stil kao ranije FAZA sekcije) - `payment_announcements.status` check prosiren na `DRAFT`/`RETURNED`, default status promenjen na `DRAFT`; nove kolone `sent_by`/`sent_at`/`returned_by`/`returned_at`/`return_reason`; retroaktivno popunjeno `sent_by`/`sent_at` za postojece OPEN/MATCHED/CANCELLED zapise (tretirani kao vec "poslati"); `documents.entity_type` check prosiren sa `PAYMENT_ANNOUNCEMENT` (za prilog uz najavu). **OVA MIGRACIJA JOS NIJE PRIMENJENA NA SUPABASE REMOTE** - potrebno je pokrenuti je (npr. `npx supabase db push` ili rucno kroz Supabase SQL editor) pre nego sto novi radni tok proradi na produkciji; bez ove izmene, backend pozivi koji upisuju status `DRAFT`/`RETURNED` ili nova polja ce pasti na constraint/kolonu koja ne postoji.
- **Backend** (`web/functions/_lib/paymentAnnouncements.js`) - `createAnnouncementCore` sad startuje na `DRAFT`; nove funkcije `updateAnnouncementCore` (izmena SAMO dok je DRAFT/RETURNED, autor ili neko sa :match privilegijom), `sendAnnouncementToCashierCore` (DRAFT/RETURNED→OPEN), `returnAnnouncementForRevisionCore` (OPEN→RETURNED, samo :match); `cancelAnnouncementCore` prosiren da dozvoljava DRAFT/OPEN/RETURNED (ranije samo OPEN); `listAnnouncementsCore` dobija `date_from`/`date_to` filtere i pravilo vidljivosti - bez eksplicitnog statusa i bez filtera po konkretnom kreatoru (npr. Knjiga merge), DRAFT/RETURNED se NE vracaju nikom (ne postoje jos za nikog osim autora).
- **Novi endpoint-i** (`web/functions/api/payment-announcements/`): `update.js`, `send.js`, `return.js` - isti obrazac kao postojeci `match.js`/`cancel.js`. `list.js` prosiren da prosledjuje `date_from`/`date_to` i (za :view/:match korisnike) proizvoljan `created_by`/`user` filter.
- **Adapter** (`cloudflare-apps-script-adapter.js`) - `apiUpdatePaymentAnnouncement`, `apiSendPaymentAnnouncementToCashier`, `apiReturnPaymentAnnouncementForRevision`; `apiListPaymentAnnouncements` prosiren sa `date_from`/`date_to`/`created_by`/`user`.
- **Frontend - deljene funkcije** (`scripts.html`, koriste ih i desktop i mobilni ekran): `openAnnouncementFormDialog_` (jedna forma za kreiranje I izmenu - Blagajna polje zakljucano pri izmeni), `openAnnouncementDetail_` prepravljen da prikazuje dugmad zavisno od statusa (Izmeni/Posalji u blagajnu za DRAFT/RETURNED, UPLATA/Vrati na doradu za OPEN, Detalji uparivanja za MATCHED, Dokumenti uvek, Otkazi kad je dozvoljeno), `refreshNajaveViews_()` osvezava koji god ekran je trenutno prikazan.
- **Frontend - desktop ekran** (`desktop.html` `#d-section-najave`, `scripts.html`) - dodat filter red (period + korisnik, `#d-najave-date-from/to`, `#d-najave-user-filter` - popunjava se istim spiskom kao Knjiga preko `renderCashbookUserFilter_`), KPI traka (Najavljeno/Uplaceno/Preostalo, `najaveKpiHtml_`), dugme "Stampaj period" (`printAnnouncementsList_`, isti print-overlay obrazac kao Blagajnicki list), kolona "Kreirao" u tabeli, status labele za Nacrt/Vracena na doradu (`.po-status-draft`, novo).
- **Frontend - mobilni ekran** (`mobile.html` `#m-section-najave`, NOV - ranije nije postojao dedikovan mobilni prikaz najava, samo FAB za kreiranje i merge u Knjigu) - card-lista (`mEntryIconBadge_` stil, isti kao Knjiga/Nalozi), period filter, KPI traka, novi nav unos u hamburger fioci (`#m-nav-najave`, vidljiv samo sa payment_announcements privilegijom). Ovo je bio NEOPHODAN dodatak, ne kozmetika: bez njega bi ANNOUNCER na mobilnom mogao da napravi DRAFT najavu ali NIKAD ne bi imao gde da je vidi/posalje (najave se vise ne pojavljuju u Knjizi dok nisu poslate, a ANNOUNCER ionako nema pristup Knjizi).
- **Dokumenti** - dugme "Dokumenti" u detalju najave otvara postojeci `openDocumentsDialog_('PAYMENT_ANNOUNCEMENT', ...)` - potpuno ponovna upotreba vec postojeceg Documents modula, jedina potrebna izmena je SQL check constraint (iznad).
- **Detalji uparivanja** - novo dugme na MATCHED najavi trazi uparenu stavku u `LAST_KNJIGA_EVENTS` (ako je trenutno ucitana u Knjizi) i otvara njen postojeci detalj-panel; ako stavka nije u trenutno ucitanom periodu, korisniku se prikazuje poruka da podesi datumski filter u Knjizi.
- **"Sve ostale forme su zakljucane" za ANNOUNCER/ASSISTANT_CASHIER** - nova `restrictNavToNajave_()` sakriva SVU navigaciju (desktop sidebar `[data-d-nav]` + mobilna hamburger fioka `[data-m-nav]`) OSIM Najava, za obe uloge, na oba ekrana - ranije se samo POCETNI ekran posle logovanja preusmeravao na Najave, ali ostala navigacija je ostajala vidljiva (samo bi 403-ovala ako bi se kliknula).

Sta NIJE uradjeno / ostaje otvoreno:

- **SQL migracija NIJE primenjena na Supabase remote** - kriticno, mora se odraditi pre koriscenja novog radnog toka (vidi napomenu iznad).
- Root uzrok povremenog neuspeha upisa pocetnog preseka pri otvaranju smene nije pronadjen (samo je greska sad vidljiva korisniku umesto da se cutke gubi).

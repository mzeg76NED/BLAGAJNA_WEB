...
import { updatePaymentRequestCore } from '../../_lib/paymentRequests.js';
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const updated = await updatePaymentRequestCore(env, requestId, data, sessionResult.session.app_user, sessionResult.session);
    return apiOk(updated);
  } catch (error) {
    return apiError(error.message || 'Izmena zahteva nije uspela.', error.status || 500);
  }
}

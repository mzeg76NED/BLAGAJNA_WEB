...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const updated = await approvePaymentRequestCore(env, requestId, data, sessionResult.session.app_user, sessionResult.session);
    return apiOk(updated);
  } catch (error) {
    return apiError(error.message || 'Odobravanje zahteva nije uspelo.', error.status || 500);
  }
}

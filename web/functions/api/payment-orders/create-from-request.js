...
import { createPaymentOrderFromRequestCore } from '../../_lib/paymentOrders.js';
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const order = await createPaymentOrderFromRequestCore(env, requestId, orderData, sessionResult.session.app_user, sessionResult.session);
    return apiOk(order);
  } catch (error) {
    return apiError(error.message || 'Kreiranje naloga iz zahteva nije uspelo.', error.status || 500);
  }
}

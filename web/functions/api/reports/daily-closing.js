...
import { getDailyClosingReportCore } from '../../_lib/reports.js';
...
export async function onRequestGet(context) {
  ...
  try {
    ...
    const rows = await getDailyClosingReportCore(env, sessionResult.session.app_user || {}, filters);
    return apiOk({ rows, count: rows.length });
  } catch (error) {
    return apiError(error.message || 'Izveštaj dnevnih zaključaka nije uspeo.', error.status || 500);
  }
}

...
import { getManagementDashboardSummaryCore } from '../../_lib/reports.js';
...
export async function onRequestGet(context) {
  ...
  try {
    ...
    const summary = await getManagementDashboardSummaryCore(env, sessionResult.session.app_user || {}, filters);
    return apiOk(summary);
  } catch (error) {
    return apiError(error.message || 'Pregled kontrolne table nije uspeo.', error.status || 500);
  }
}

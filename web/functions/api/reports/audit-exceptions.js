...
import { getAuditExceptionsReportCore } from '../../_lib/reports.js';
...
export async function onRequestGet(context) {
  ...
  try {
    ...
    const rows = await getAuditExceptionsReportCore(env, sessionResult.session.app_user || {}, filters);
    return apiOk({ rows, count: rows.length });
  } catch (error) {
    return apiError(error.message || 'Izveštaj izuzetaka nije uspeo.', error.status || 500);
  }
}
